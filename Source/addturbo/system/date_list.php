...
"><a href="index.php?page=total_money" class="btn btn-default fa fa-arrow-left"></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="index.php?page=date_list&id=<?php echo $_GET['id']; ?>&comment=<?php echo $_GET['comment']; ?>&date_money=<?php echo $_GET['date_money']; ?>" class="btn btn-default fa fa-rotate-right"></a> </span><br><br>
                       <div class="table-responsive">
						<table class="table table-striped table-hover"  id="dataTables-example">
							...
							$no=0;$total=0;
							$sql=$db->DB->prepare("SELECT * FROM mt_gen WHERE money_code='".$money_code."'");
	                      $sql->execute();
                           while($result = $sql->fetch( PDO::FETCH_ASSOC ))	{
														...
														echo "<tr >";
															...
															echo "<td class=\"text-center\">".$convert->Convert_time($result['comment'])."</td>";
															echo "<td>";
...
$con2_arr=($con1_arr)."/".($d_arr)."/".($y_arr);
		   ...
		   $rows=$db->rows_num("SELECT * FROM mt_money WHERE money_code='".$money_code."'");
		   if($rows==0){
		 $db->add_db("mt_money",array(
				                    "utc_time_for_chart" =>$utc_data."".$id_code,
