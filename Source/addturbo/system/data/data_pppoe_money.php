...
$API = new routeros_api();
 ...
 #############################################################################################
 $sql=$db->DB->prepare("SELECT * FROM pppoe_money_year WHERE mt_id='".$id."'");
	                                   $sql->execute();
									while($result = $sql->fetch( PDO::FETCH_ASSOC ))	{
     $y_data=substr("".$result['year']."",-20,4); 
    $thaiyear=($y_data+543);
	$jan=$result['jan'];
	$money_jan=round(substr("".$jan."",-20),2);
	$drilldown_jan="".$y_data."".substr("".$jan."",-4)."-id".$id."";
	if($money_jan==0){$drilldown_jan=null;$money_jan=null;}
	$data_jan=array('name'=>'มกราคม','y'=>$money_jan,'drilldown'=>$drilldown_jan);
    

	$feb=$result['feb'];
	$money_feb=round(substr("".$feb."",-20),2);
	$drilldown_feb="".$y_data."".substr("".$feb."",-4)."-id".$id."";
	 if($money_feb==0){$drilldown_feb=null;$money_feb=null;}
	$data_feb=array('name'=>'กุมภาพันธ์','y'=>$money_feb,'drilldown'=>$drilldown_feb);
   

	$mar=$result['mar'];
	$money_mar=round(substr("".$mar."",-20),2);
	$drilldown_mar="".$y_data."".substr("".$mar."",-4)."-id".$id."";
	  if($money_mar==0){$drilldown_mar=null;$money_mar=null;}
	$data_mar=array('name'=>'มีนาคม','y'=>$money_mar,'drilldown'=>$drilldown_mar);
   

	$apr=$result['apr'];
	$money_apr=round(substr("".$apr."",-20),2);
	$drilldown_apr="".$y_data."".substr("".$apr."",-4)."-id".$id."";
	if($money_apr==0){$drilldown_apr=null;$money_apr=null;}
	$data_apr=array('name'=>'เมษายน','y'=>$money_apr,'drilldown'=>$drilldown_apr);
  

	$may=$result['may'];
	$money_may=round(substr("".$may."",-20),2);
	$drilldown_may="".$y_data."".substr("".$may."",-4)."-id".$id."";
	if($money_may==0){$drilldown_may=null;$money_may=null;}
	$data_may=array('name'=>'พฤษภาคม','y'=>$money_may,'drilldown'=>$drilldown_may);
  

	$jun=$result['jun'];
	$money_jun=round(substr("".$jun."",-20),2);
	$drilldown_jun="".$y_data."".substr("".$jun."",-4)."-id".$id."";
	if($money_jun==0){$drilldown_jun=null;$money_jun=null;}
	$data_jun=array('name'=>'มิถุนายน','y'=>$money_jun,'drilldown'=>$drilldown_jun);
   

	$jul=$result['jul'];
	$money_jul=round(substr("".$jul."",-20),2);
	$drilldown_jul="".$y_data."".substr("".$jul."",-4)."-id".$id."";
	if($money_jul==0){$drilldown_jul=null;$money_jul=null;}
	$data_jul=array('name'=>'กรกฎาคม','y'=>$money_jul,'drilldown'=>$drilldown_jul);
  

	$aug=$result['aug'];
	$money_aug=round(substr("".$aug."",-20),2);
	$drilldown_aug="".$y_data."".substr("".$aug."",-4)."-id".$id."";
	if($money_aug==0){$drilldown_aug=null;$money_aug=null;}
	$data_aug=array('name'=>'สิงหาคม','y'=>$money_aug,'drilldown'=>$drilldown_aug);


	$sep=$result['sep'];
	$money_sep=round(substr("".$sep."",-20),2);
	$drilldown_sep="".$y_data."".substr("".$sep."",-4)."-id".$id."";
	if($money_sep==0){$drilldown_sep=null;$money_sep=null;}
	$data_sep=array('name'=>'กันยายน','y'=>$money_sep,'drilldown'=>$drilldown_sep);
   

	$oct=$result['oct'];
	$money_oct=round(substr("".$oct."",-20),2);
	$drilldown_oct="".$y_data."".substr("".$oct."",-4)."-id".$id."";
	if($money_oct==0){$drilldown_oct=null;$money_oct=null;}
	$data_oct=array('name'=>'ตุลาคม','y'=>$money_oct,'drilldown'=>$drilldown_oct);
  

	$nov=$result['nov'];
	$money_nov=round(substr("".$nov."",-20),2);
	$drilldown_nov="".$y_data."".substr("".$nov."",-4)."-id".$id."";
	 if($money_nov==0){$drilldown_nov=null;$money_nov=null;}
	$data_nov=array('name'=>'พฤศจิกายน','y'=>$money_nov,'drilldown'=>$drilldown_nov);
 

	$december=$result['december'];
	$money_december=round(substr("".$december."",-20),2);
	$drilldown_december="".$y_data."".substr("".$december."",-4)."-id".$id."";
	if($money_december==0){$drilldown_december=null;$money_december=null;}
	$data_december=array('name'=>'ธันวาคม','y'=>$money_december,'drilldown'=>$drilldown_december);
...
$data= array($data_jan,$data_feb,$data_mar,$data_apr,$data_may,$data_jun,$data_jul,$data_aug,$data_sep,$data_oct,$data_nov,$data_december);
	...
	$year_data[]=array('name'=>''.$thaiyear.'','y'=>$year_money,'drilldown'=>$result['year']);	
		
	
	$series0['series'][]=array('colorByPoint'=>true,'name'=>'YEARS','data'=>$year_data);
	

	$series1['drilldown'][]=array('colorByPoint'=>true,'name'=>''.$thaiyear.'','id'=>$result['year'],'data'=>$data);

	}

################################################
  $sql=$db->DB->prepare("SELECT * FROM pppoe_money_month WHERE mt_id='".$id."'");
	                                   $sql->execute();
									while($result = $sql->fetch( PDO::FETCH_ASSOC ))	{
      ...
      $month_data_arr=array("jan"=>"มกราคม","feb"=>"กุมภาพันธ์","mar"=>"มีนาคม","apr"=>"เมษายน","may"=>"พฤษภาคม","jun"=>"มิถุนายน","jul"=>"กรกฎาคม","aug"=>"สิงหาคม","sep"=>"กันยายน","oct"=>"ตุลาคม","nov"=>"พฤศจิกายน","dec"=>"ธันวาคม");
       $thai_month=$month_data_arr[$month_data];
	   $thai_year_arr=($year_data+543);
...
$series1['drilldown'][]=array('name'=>''.$thai_month.' '.$thai_year_arr.'','id'=>$result['month_code'],'data'=>$drilldown_arr);
	 	

	}

	$year_data_empty[]=array('name'=>'Empty','y'=>0,'drilldown'=>null);
   $series0_empty['series'][]=array('colorByPoint'=>true,'name'=>'YEARS','data'=>$year_data_empty);
    $series1_empty['drilldown'][]=array('colorByPoint'=>true,'name'=>'Empty','id'=>0,'data'=>0);
	$data_title['title']=array('text'=>'PPPOE Money Chart');
	$nodata_title['title']=array('text'=>'NO DATA! MONEY IN PPPOE DATABASE');
  $data_subtitle['subtitle']=array('text'=>'สถิติ รวมยอดรายรับประจำเดือน');
##############################################
}
if(!empty($series0)){
$result = array();
