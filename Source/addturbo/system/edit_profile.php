<?php
    ...
    window.location.href = 'index.php?page=profilelist';}})</script>";
			...
			$num=$db->rows_num("SELECT pro_name FROM mt_profile where pro_name='".$_REQUEST['pro_name']."'");
...
<div class="row">
		...
		$rows=$db->rows_num("SELECT * FROM mt_profile WHERE pro_name='".$_GET['name']."'");
		if($rows >0){
                    ...
                    <?php }else{?>    
							<div class="col-xs-12 col-md-6">
                                <div class="form-group">
                                    <span class=" style1">Price <img src="../img/help.png" width="16" height="16"  class="no1" data-toggle="tooltip" data-placement="right" title="กำหนดราคา ของ profile"></span>
                                    <option name="price" type="text" placeholder="Ex.150" class="form-control" value="<?php echo $result['pro_price']; ?>">...Empty profile on database...</option>
                               </div>
							</div>
					<?php }?>	
						</div>
