<?php
   ...
   if(!empty($_REQUEST['active'])){
	...
	$sql=$db->DB->prepare("SELECT * FROM mt_edit WHERE group_code='".$group_code."'");
		...
		$rows=$db->rows_num("SELECT * FROM pppoe_pro WHERE pro_name='".$result['user']."'");
		$num =count($ARRAY);
	for($i=0; $i<$num; $i++){													
...
$sql=$db->DB->prepare("SELECT * FROM mt_edit WHERE group_code='".$group_code."'");
	...
	<?php 
	$profile=$db->selectquery("SELECT * FROM pppoe_pro WHERE pro_name='".$result['user']."'");
	...
	?>
               ...
               echo $convert->button_btn_submit_account($account,$text_success,$bottonbtn_success,'','','','','');
				...
				<button id="btnCancel" class="btn btn-danger" type="reset"  Onclick="window.location.href = 'index.php?page=pppoe_profile_list&cancel=yes'"><i class="fa fa-times"></i>&nbsp;Cancel</button>
