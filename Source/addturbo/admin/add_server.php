...
echo "<script language='javascript'>swal('ERROR SITE PIN !','กำหนด Site PIN อย่างน้อย จำนวน 1 ไซต์ และไม่เหมือนกัน','error').then(function () {
   window.history.back();}, function (dismiss) {if (dismiss === 'overlay') {
	...
	$num = $db->rows_num("SELECT * FROM mt_config where mt_ip='".$ip."' or admin_pin='".md5($cus_pin)."' or admin_pin='".md5($user_pin)."'
	".$cusPIN_data." ".$userPIN_data.""); 
     
	 if($num==0){
	   $db->add_db("mt_config",array(
        "mt_user"  =>  $user,
        "mt_pass"  =>  $pass,
		"mt_ip"    =>   $ip,
		"port_api"  =>  $papi,
  ...
  if (dismiss === 'overlay') {
   ...
   if($num==0){
	   ...
	   "site_name"  =>  $sitename,
		...
		$last_id=$db->new_insert_id();
		$update = $db->update_db("mt_config",array(
          "mt_id"=>$last_id,
        ),"mt_ip='".$ip."'");     
...
?>
	  ...
	  <div class="<?php print $convert->panel_modify();?>">
          ...
          <div class="modal-body">
            ...
            <input type="text" class="form-control" id="customerPin" name="customerPin"  title="Please enter your Site PIN" maxlength="12"  placeholder="Ex.12345678" >
                                 </div> 
                              </div>
			<div class="col-xs-12  col-md-6">
							  <div class="form-group">
            ...
            <input type="text" class="form-control" id="userPin" name="userPin"  title="Please enter your Site PIN" maxlength="12"  placeholder="Ex.12345678" >
                                 </div> 
                              </div>
                              <?php  }else{
							  if($secom_v2==$_SESSION['security']){
                    ...
                    <label for="password" class="control-label">  Site PIN </label>
                  <a href="#"  data-toggle="modal" data-target="#PINDetail" data-toggle="tooltip" data-placement="top" title="ดูรายละเอียด"><i class="fa fa-circle" style="color: #f7d13c"> </i></a>
                                  <option type="text" class="form-control" id="customerPin" name="customerPin"  title="Please enter your Site PIN" maxlength="12" value="<?php echo $_SESSION['security'];?>" placeholder="Ex.12345678"  required>
                   ...
                   <a href="#"  data-toggle="modal" data-target="#PINDetail" data-toggle="tooltip" data-placement="top" title="ดูรายละเอียด"><i class="fa fa-circle" style="color: #f7d13c"> </i></a> 
                    ...
                    <label for="password" class="control-label"> Site PIN </label>
                  <a href="#"  data-toggle="modal" data-target="#PINDetail" data-toggle="tooltip" data-placement="top" title="ดูรายละเอียด"><i class="fa fa-circle" style="color: #ff1c15"> </i></a>
                                  <option type="text" class="form-control" id="userPin" name="userPin"  title="Please enter your Site PIN" maxlength="12" value="<?php echo $_SESSION['security'];?>" placeholder="Ex.12345678"  required>
                                 <?php echo $_SESSION['security'];?> 
								  </div> 
                              </div>
