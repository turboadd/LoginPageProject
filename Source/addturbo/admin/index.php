 <?php
	...
	if(empty($_SESSION['security']) || empty($_SESSION['APIUser'])){
		...
		// include("convert.php");
	      ...
	      $db = new ConnectDB();
	      $secom_customer=$db->DB->prepare("SELECT * FROM mt_config ");
            $secom_customer->execute();
             while($ADMINaccount = $secom_customer->fetch( PDO::FETCH_ASSOC )){
			  
			   
			    
	if($ADMINaccount['admin_pin']==$_SESSION['security']){$security_account="style=\"color: #00ff00;\"";
	                             $secom_v1=$ADMINaccount['admin_pin'];}
	if($ADMINaccount['customer_pin']==$_SESSION['security']){$security_account="style=\"color: #f7d13c;\"";
	                              $secom_v2=$ADMINaccount['customer_pin'];}
	if($ADMINaccount['user_pin']==$_SESSION['security']){$security_account="style=\"color: #ff1c15;\"";}
	                              $secom_v3=$ADMINaccount['user_pin'];
		   }
...
<head>
	...
	<script async src="https://www.googletagmanager.com/gtag/js?id=UA-113460060-4"></script>
	<script>
	  window.dataLayer = window.dataLayer || [];
	  function gtag(){dataLayer.push(arguments);}
	  gtag('js', new Date());

	  gtag('config', 'UA-113460060-4');
	</script>
	<!-- Global site tag (gtag.js) - Google Analytics -->
...
<div class="wrapper">
  ...
  <header class="main-header">
    ...
    <a href="index.php" class="logo">
      ...
      <span class="logo-lg"><b>NT </b>API</span>
    </a>
...
<ul class="sidebar-menu">
        ...
        <li class="active treeview">
          <a href="index.php">
            <i class="fa fa-dashboard"></i> <span>Mikrotik Deviced</span>
...
else if($_REQUEST['page']=="server"){include("listserver.php");}
