<?php
  ...
  if(empty($_REQUEST['customerPin']) && empty($_REQUEST['userPin'])){
   ...
   window.history.back();}, function (dismiss) {if (dismiss === 'overlay') {
	...
	$num = $db->rows_num("SELECT * FROM mt_config where admin_pin='".md5($_REQUEST['userPin'])."'or admin_pin='".md5($_REQUEST['customerPin'])."' ".$cusPIN_data." ".$userPIN_data."");
          ...
          <div class="modal-body">
              <div class="row">
                  <div class="col-xs-7">
                      <div class="well">
					  <?php
							$result=$db->selectquery("SELECT * FROM mt_config WHERE mt_num='".$_GET['id']."'");
