<?php
	...
	$login="{:delay 1;:local m [ip hotspot active get value-name=mac-address [find user=\$username]];:local addmac [/ip hotspot user get \$username mac-address];:local whouser \$user;:local whoip \$address;:local date [/system clock get date];:local time [/system clock get time];:log info \"user logged in: \$whouser IP: \$whoip Mac: \$m\";:if ([/ip hotspot user get \$whouser comment ]=\"\") do={[/ip hotspot user set \$whouser comment=\"\$date \$time\"];:log info \"New Hotspot user logged in: \$whouser\";};:if (\$addmac != \$m) do={[/ip hotspot user set mac-address=\$m [find name=\$whouser]];:log info \"Add mac address : \$m to user: \$whouser successfully\";}}";
